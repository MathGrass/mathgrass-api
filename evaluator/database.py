...
class Database:
    ...
    def _get_edges(self, graph_id):
        ...
        ecur.execute("SELECT * FROM graphs_edges AS ge INNER JOIN edges AS e ON e.id = ge.edges_id "
    ...
    def _get_vertices(self, graph_id):
        ...
        vcur.execute("SELECT * FROM graphs_vertices AS gv INNER JOIN vertices AS v ON v.id = gv.vertices_id "
                     "WHERE graph_entity_id = " + str(graph_id))
    ...
    @staticmethod
    def get_cursor_elements_as_dicts(cursor):
